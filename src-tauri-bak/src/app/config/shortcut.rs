...
use crate::infrastructure::services::capture::window_cap::capture_screen;
    ...
    F: Fn(&AppHandle, &Shortcut, ShortcutEvent) + Send + Sync + 'static,
{
    let shortcut = Shortcut::from_str(shortcut_str)
        .map_err(|e| AppError::ConfigError(format!("解析{}快捷键 '{}' 失败：{}", description, shortcut_str, e)))?;

    manager.on_shortcut(shortcut, handler)
        .map_err(|e| AppError::ConfigError(format!("绑定{}快捷键 '{}' 失败：{}", description, shortcut_str, e)))?;
...
pub fn unregister_all(app_handle : &AppHandle) -> AppResult<()>{
    app_handle.global_shortcut().unregister_all()
        .map_err(|e| AppError::ConfigError(format!("卸载所有快捷键失败：{}",e)))?;
    Ok(())
}
