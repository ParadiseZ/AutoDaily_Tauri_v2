use crate::app::config::shortcut::{register_by_config, unregister_all};
    ...
    if old_enabled != new_enabled {
        if new_enabled {
            ...
            if let Some(main_window) = app_handle.get_webview_window(MAIN_WINDOW) {
                ...
                println!("窗口状态记忆功能已启用，状态已恢复");
            }
        } else {
            // 禁用：保存一次当前状态然后清除缓存文件（可选）
            use tauri_plugin_window_state::{AppHandleExt, StateFlags};
            let _ = app_handle.save_window_state(StateFlags::all());
            println!("窗口状态记忆功能已禁用，当前状态已保存");
        }
    }
    Ok(())
}
    ...
    #[cfg(desktop)]
    {
        ...
        match autostart_manager.is_enabled() {
                ...
                if current_enabled != auto_start {
                    if auto_start {
                        if let Err(e) = autostart_manager.enable() {
                            ...
                            return Err(AppError::ConfigError(format!("启用开机自启动失败: {}", e)));
                        } else {
                            Log::info("开机自启动已启用");
                        }
                    } else {
                        if let Err(e) = autostart_manager.disable() {
                            ...
                            return Err(AppError::ConfigError(format!("禁用开机自启动失败: {}", e)));
                        } else {
                            Log::info("开机自启动已禁用");
                        }
                    }
            ...
            Err(e) => {
                ...
                return Err(AppError::ConfigError(format!("无法检查开机自启动状态: {}", e)));
            }
        }
    }
...
pub async fn save_window_state_if_enabled(
    ...
    let system_settings = manager.get_conf::<SystemConfig>(SYSTEM_SETTINGS_PATH).await?;
    if system_settings.rem_size_position {
        use tauri_plugin_window_state::{AppHandleExt, StateFlags};
        let _ = get_app_handle().await.save_window_state(StateFlags::all());
        Log::info("窗口状态已保存");
    }
    Ok(())
}