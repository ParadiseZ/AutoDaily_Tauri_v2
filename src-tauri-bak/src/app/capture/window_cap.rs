...
pub fn save_screenshot(
    ...
    Log::fn_begin("save_screenshot");
    Log::info_with_fields(
        "保存截图",
        vec![
            ("设备名称", device_name.to_string()),
            ("图像类型", image_type.to_string()),
        ],
    );
...
pub fn save_base64_image(base64_image: &str, _title: &str) -> Result<String, String> {
    Log::fn_begin("save_base64_image");
    ...
    match fs::write(&file_path, image_data) {
        Ok(_) => {
            let success_msg = format!("图像已保存到: {}", path_str);
            Log::info(&success_msg);
            Log::fn_end("save_base64_image");
            Ok(path_str)
        }
        Err(e) => {
            let error_msg = format!("保存图像失败: {:?}", e);
            Log::error(&error_msg);
            Log::fn_end("save_base64_image");
            Err(error_msg)
        }
