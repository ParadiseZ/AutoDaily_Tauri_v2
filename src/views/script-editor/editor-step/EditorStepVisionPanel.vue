<template>
  <div class="space-y-4">
    ...
    <template v-else-if="selectedVision.type === VISION_TYPE.visionSearch">
...
const selectedInputVar = computed(() =>
  props.selectedVision.type === VISION_TYPE.visionSearch ? '' : props.selectedVision.input_var,
);
const selectedVisionSearch = computed(() =>
  props.selectedVision.type === VISION_TYPE.visionSearch ? props.selectedVision : null,
);
...
const resolvedVisionSearchOcrInputOptions = computed(() =>
  buildNullableStructuredOptions(
    ...
    selectedVisionSearch.value?.ocr_res_var ?? '',
...
const resolvedVisionSearchDetInputOptions = computed(() =>
  buildNullableStructuredOptions(
    props.readableCatalogVariableOptions ?? [],
    selectedVisionSearch.value?.det_res_var ?? '',
    '留空时使用当前上下文中的最近检测结果。',
  ),
);
...
const resolvedVisionSearchDetOutputOptions = computed(() =>
  buildNullableStructuredOptions(
    ...
    selectedVisionSearch.value?.out_det_var ?? '',
    '留空时不写出检测筛选结果。',
  ),
);
...
const resolvedVisionSearchOcrOutputOptions = computed(() =>
  buildNullableStructuredOptions(
    props.writableCatalogVariableOptions ?? [],
    selectedVisionSearch.value?.out_ocr_var ?? '',
    '留空时不写出 OCR 筛选结果。',
  ),
);
</script>
