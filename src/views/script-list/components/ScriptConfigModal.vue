...
<template>
  <dialog class="modal" :class="{ 'modal-open': isOpen }">
    <div class="modal-box w-11/12 max-w-4xl bg-base-100 p-0 overflow-hidden border border-base-content/10 flex flex-col max-h-[85vh]">
      <!-- Header -->
        ...
        <div class="grow overflow-y-auto custom-scrollbar p-6">
            ...
            <div v-show="activeTab === 'basic'" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div class="form-control hover:bg-base-200/50 p-2 rounded-lg transition-colors">
                    <label class="label"><span class="label-text font-bold">名称 <span class="text-error">*</span></span></label>
                    <input type="text" v-model="formState.name" placeholder="例如：每日委托自动执行" class="input input-bordered w-full" />
                </div>
                <div class="form-control hover:bg-base-200/50 p-2 rounded-lg transition-colors">
                    <label class="label"><span class="label-text font-bold">包名/主Activity<span class="text-error">*</span></span></label>
                    <input type="text" v-model="formState.pkgName" placeholder="com.example.game" class="input input-bordered w-full" />
                </div>
                <div class="form-control hover:bg-base-200/50 p-2 rounded-lg transition-colors">
                    <label class="label"><span class="label-text font-bold">描述</span></label>
                    <textarea v-model="formState.description" class="textarea textarea-bordered h-24"></textarea>
                </div>
            </div>
            ...
            <div v-show="activeTab === 'det'" class="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <section class="space-y-4">
                    <div class="flex items-center justify-between border-b border-base-content/10 pb-2">
                        <h4 class="font-bold flex items-center gap-2">
                            <Eye class="w-4 h-4 text-primary" /> 图像/目标检测模型
                        </h4>
                    ...
                    <div v-if="formState.imgDetModelType === 'Yolo11'" class="bg-base-200/50 p-4 rounded-xl space-y-4 border border-base-content/5">
                        <div class="grid grid-cols-2 gap-4">
                            <!-- Base Model Param Common -->
                            <div class="form-control col-span-2">
                                <label class="label text-xs font-bold opacity-70">模型路径 (ONNX)</label>
                                <div class="flex gap-2">
                                     <input type="text" v-model="formState.yoloParams.modelPath" class="input input-sm input-bordered grow font-mono text-xs" placeholder="/path/to/model.onnx" />
                                     <button class="btn btn-sm btn-square btn-ghost border border-base-content/20">...</button>
                                </div>
                            </div>
                            <div class="form-control col-span-2">
                                <label class="label text-xs font-bold opacity-70">标签路径 (Label Path)</label>
                                <div class="flex gap-2">
                                     <input type="text" v-model="formState.yoloParams.labelPath" class="input input-sm input-bordered grow font-mono text-xs" placeholder="/path/to/config.yaml" />
                                     <button class="btn btn-sm btn-square btn-ghost border border-base-content/20">...</button>
                                </div>
                            </div>
                            ...
                            <div class="form-control">
                                ...
                                <select v-model="formState.yoloParams.executionProvider" class="select select-bordered select-sm text-xs">
                                    <option v-for="p in executionProviders" :key="p" :value="p">{{ p }}</option>
                                </select>
                            </div>
                            <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">输入图像宽高 (WxH)</label>
                                <div class="flex items-center gap-2">
                                    <input type="number" v-model="formState.yoloParams.inputWidth" class="input input-sm input-bordered w-full text-xs" />
                                    <span class="opacity-50">x</span>
                                    <input type="number" v-model="formState.yoloParams.inputHeight" class="input input-sm input-bordered w-full text-xs" />
                                </div>
                            </div>
                            <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">类别数量:</label>
                                <input type="text" disabled  v-model="formState.yoloParams.classCount" class="input input-sm input-bordered text-xs" />
                            </div>
                            </div>
                             <div class="form-control">
                                ...
                                <input type="range" min="0" max="1" step="0.01" v-model.number="formState.yoloParams.confidenceThresh" class="range range-xs range-secondary" />
                            </div>
                             <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">
                                    <span>IOU 阈值: {{ formState.yoloParams.iouThresh }}</span>
                                </label>
                                <input type="range" min="0" max="1" step="0.01" v-model.number="formState.yoloParams.iouThresh" class="range range-xs range-primary" />
                            </div>
                        </div>
                </section>
                ...
                <!-- Txt Det Section -->
                 <section class="space-y-4">
                    ...
                    <!-- DBNet Params -->
                    <div v-if="formState.txtDetModelType === 'PaddleDbNet'" class="bg-base-200/50 p-4 rounded-xl space-y-4 border border-base-content/5">
                        <div class="grid grid-cols-2 gap-4">
                             <div class="form-control col-span-2">
                                <label class="label text-xs font-bold opacity-70">模型路径 (ONNX)</label>
                                <div class="flex gap-2">
                                     <input type="text" v-model="formState.dbNetParams.modelPath" class="input input-sm input-bordered grow font-mono text-xs" placeholder="/path/to/dbnet.onnx" />
                                     <button class="btn btn-sm btn-square btn-ghost border border-base-content/20">...</button>
                                </div>
                            </div>
                             <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">执行器 (Provider)</label>
                                <select v-model="formState.dbNetParams.executionProvider" class="select select-bordered select-sm text-xs">
                                    <option v-for="p in executionProviders" :key="p" :value="p">{{ p }}</option>
                                </select>
                            </div>
                            <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">输入尺寸 (WxH)</label>
                                <div class="flex items-center gap-2">
                                    <input type="number" v-model="formState.dbNetParams.inputWidth" class="input input-sm input-bordered w-full text-xs" />
                                    <span class="opacity-50">x</span>
                                    <input type="number" v-model="formState.dbNetParams.inputHeight" class="input input-sm input-bordered w-full text-xs" />
                                </div>
                            </div>
                            <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">
                                ...
                                <input type="range" min="0" max="1" step="0.01" v-model.number="formState.dbNetParams.dbThresh" class="range range-xs range-secondary" />
                            </div>
                            <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">
                                    <span>框扩充比例 (Unclip): {{ formState.dbNetParams.unclipRatio }}</span>
                                </label>
                                <input type="range" min="1.0" max="4.0" step="0.1" v-model.number="formState.dbNetParams.unclipRatio" class="range range-xs range-secondary" />
                            </div>
                    ...
                    <div v-if="formState.txtDetModelType === 'Yolo11'" class="bg-base-200/50 p-4 rounded-xl space-y-4 border border-base-content/5">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="form-control col-span-2">
                                <label class="label text-xs font-bold opacity-70">模型路径 (ONNX)</label>
                                <div class="flex gap-2">
                                     <input type="text" v-model="formState.txtDetYoloParams.modelPath" class="input input-sm input-bordered grow font-mono text-xs" placeholder="/path/to/text_det_yolo.onnx" />
                                     <button class="btn btn-sm btn-square btn-ghost border border-base-content/20">...</button>
                                </div>
                            </div>
                            <div class="form-control col-span-2">
                                <label class="label text-xs font-bold opacity-70">标签路径 (Label Path)</label>
                                <div class="flex gap-2">
                                     <input type="text" v-model="formState.txtDetYoloParams.labelPath" class="input input-sm input-bordered grow font-mono text-xs" placeholder="/path/to/config.yaml" />
                                     <button class="btn btn-sm btn-square btn-ghost border border-base-content/20">...</button>
                                </div>
                            </div>
                            <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">执行器 (Provider)</label>
                                <select v-model="formState.txtDetYoloParams.executionProvider" class="select select-bordered select-sm text-xs">
                                    <option v-for="p in executionProviders" :key="p" :value="p">{{ p }}</option>
                                </select>
                            </div>
                             <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">输入尺寸 (WxH)</label>
                                <div class="flex items-center gap-2">
                                    <input type="number" v-model="formState.txtDetYoloParams.inputWidth" class="input input-sm input-bordered w-full text-xs" />
                                    <span class="opacity-50">x</span>
                                    <input type="number" v-model="formState.txtDetYoloParams.inputHeight" class="input input-sm input-bordered w-full text-xs" />
                                </div>
                            </div>
                             <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">类别数量 (Class Count)</label>
                                <input type="text" disabled  v-model="formState.txtDetYoloParams.classCount" class="input input-sm input-bordered text-xs" />
                            </div>
                            
                            <!-- Text Index Specific Parameter -->
                            <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">文本类别索引 (Text Class Index)</label>
                                <input type="number" v-model="formState.txtDetYoloParams.txtIdx" class="input input-sm input-bordered text-xs" placeholder="0" />
                             ...
                             <div class="form-control">
                                ...
                                <input type="range" min="0" max="1" step="0.01" v-model.number="formState.txtDetYoloParams.confidenceThresh" class="range range-xs range-secondary" />
                            </div>
                             <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">
                                    <span>IOU 阈值: {{ formState.txtDetYoloParams.iouThresh }}</span>
                                </label>
                                <input type="range" min="0" max="1" step="0.01" v-model.number="formState.txtDetYoloParams.iouThresh" class="range range-xs range-secondary" />
                            </div>
                        </div>
                    </div>
                 </section>
            ...
            <div v-show="activeTab === 'rec'" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                  <div class="flex items-center justify-between border-b border-base-content/10 pb-2">
                        <h4 class="font-bold flex items-center gap-2">
                            <Type class="w-4 h-4 text-accent" /> 文本识别模型
                        </h4>
                    ...
                    <div v-if="formState.txtRecModelType === 'PaddleCrnn'" class="bg-base-200/50 p-4 rounded-xl space-y-4 border border-base-content/5">
                        <div class="grid grid-cols-2 gap-4">
                             <div class="form-control col-span-2">
                                <label class="label text-xs font-bold opacity-70">模型路径 (ONNX)</label>
                                <div class="flex gap-2">
                                     <input type="text" v-model="formState.crnnParams.modelPath" class="input input-sm input-bordered grow font-mono text-xs" placeholder="/path/to/rec_model.onnx" />
                                     <button class="btn btn-sm btn-square btn-ghost border border-base-content/20">...</button>
                                </div>
                            </div>
                            <div class="form-control col-span-2">
                                ...
                                <div class="flex gap-2">
                                     <input type="text" v-model="formState.crnnParams.dictPath" class="input input-sm input-bordered grow font-mono text-xs" placeholder="/path/to/keys.txt" />
                                     <button class="btn btn-sm btn-square btn-ghost border border-base-content/20">...</button>
                                </div>
                            </div>
                             <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">执行器 (Provider)</label>
                                <select v-model="formState.crnnParams.executionProvider" class="select select-bordered select-sm text-xs">
                                    <option v-for="p in executionProviders" :key="p" :value="p">{{ p }}</option>
                                </select>
                            </div>
                            <div class="form-control">
                                <label class="label text-xs font-bold opacity-70">输入尺寸 (WxH)</label>
                                <div class="flex items-center gap-2">
                                    <input type="number" v-model="formState.crnnParams.inputWidth" class="input input-sm input-bordered w-full text-xs" />
                                    <span class="opacity-50">x</span>
                                    <input type="number" v-model="formState.crnnParams.inputHeight" class="input input-sm input-bordered w-full text-xs" />
                                </div>
                            </div>
                        </div>
                    </div>
                    
                    <div v-if="formState.imgDetModelType === 'None' && formState.txtDetModelType === 'None' && formState.txtRecModelType === 'None'" 
      ...
      <div class="p-4 border-t border-base-content/5 bg-base-200/30 flex justify-end gap-3 flex-none">
        ...
        <button @click="handleSave" class="btn btn-primary px-8" :disabled="!formState.name">创建</button>
      </div>
    </div>
