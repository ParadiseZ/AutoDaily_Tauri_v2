...
impl DeviceRuntime {
    pub fn platform(&self) -> DevicePlatform {
        match self {
            Self::Android(_) => DevicePlatform::Android,
            Self::Desktop(_) => DevicePlatform::Desktop,
        }
    }
    ...
    pub async fn change_cap_method(&self, method: CaptureMethod) -> bool {
        match self {
            Self::Android(runtime) => runtime.change_cap_method(method).await,
            Self::Desktop(runtime) => runtime.change_cap_method(method).await,
        }
    }
...
#[derive(Clone)]
pub struct AndroidDeviceRuntime {
...
impl AndroidDeviceRuntime {
    ...
    pub async fn execute_operation(&self, operation: DeviceOperation) -> Result<(), String> {
        match operation {
            ...
            DeviceOperation::InputText(text) => {
                self.send_command(ADBCommand::InputText(text))
            }
...
#[derive(Clone)]
pub struct DesktopDeviceRuntime;
...
impl DesktopDeviceRuntime {
    ...
    pub async fn change_cap_method(&self, method: CaptureMethod) -> bool {
        Log::warn(&format!(
            "DesktopDeviceRuntime 尚未实现截图方式切换，忽略请求: {}",
            method
        ));
        false
    }
    ...
    #[allow(dead_code)]
    pub async fn unsupported_action(&self, action: &str) -> Result<(), String> {
