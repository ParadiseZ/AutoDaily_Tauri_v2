...
use ad_kernel::ids::{AssignmentId, DeviceId, TaskId};
use chrono::{DateTime, Datelike, Duration, Local};
use domain_script::TaskCycle;
use domain_script::{ScriptTaskProfile, TaskRowType, TaskTriggerMode};
use runner_protocol::message::{RunTarget, RuntimeQueueItem};
use serde::Deserialize;
use std::collections::HashMap;

pub(crate) struct ExecutionPlanAssembler;
...
pub(crate) struct PlannedTask {
    pub task: ScriptTaskProfile,
    pub task_cycle: TaskCycle,
    pub record_schedule: bool,
}

#[derive(Debug, Clone)]
pub(crate) struct SkippedTask {
    pub task: ScriptTaskProfile,
    pub task_cycle: TaskCycle,
    pub reason: String,
    pub record_schedule: bool,
...
impl ExecutionPlanAssembler {
    ...
    pub(crate) fn is_policy_debug_target(run_target: &RunTarget) -> bool {
        matches!(
            run_target,
    ...
    pub(crate) fn should_record_schedule(run_target: &RunTarget, task: &ScriptTaskProfile) -> bool {
    ...
    fn resolve_trigger_mode(task: &ScriptTaskProfile) -> (bool, bool) {
    ...
    fn resolve_task_cycle(
        ...
        template_values
            ...
            .unwrap_or_else(|| task.default_task_cycle.clone())
    }
...
impl ExecutionPlan {
    pub(crate) fn task_selection(&self) -> TaskSelection {
        match self {
    ...
    pub(crate) fn is_policy_debug(&self) -> bool {
        matches!(self, ExecutionPlan::PolicyDebug)
    }

    pub(crate) fn summary(&self) -> ExecutionPlanSummary {
        match self {
    ...
    pub(crate) fn progress_message(&self) -> String {
