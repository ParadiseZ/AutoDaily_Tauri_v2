impl ScriptExecutor {
    ...
    async fn resolve_exec_limit(&self, target: &StateTarget) -> ExecuteResult<Option<u32>> {
        ...
        match target {
            StateTarget::Task { id } => {
                let tasks: Vec<ScriptTaskProfile> =
            ...
            StateTarget::Policy { id } => {
                let policies: Vec<PolicyProfile> = Self::parse_bundle_json(
                    "condition.execNumCompare",
                    "policies_json",
                ...
                Ok((policy.info.exec_max > 0).then_some(u32::from(policy.info.exec_max)))
            }
        }
    }
