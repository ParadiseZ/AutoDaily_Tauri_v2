impl ScriptExecutor {
    ...
    async fn resolve_policy_set_candidates(
        ...
        let policy_map: HashMap<PolicyId, PolicyProfile> = bundle
            .policies
            .iter()
            .cloned()
            .map(|policy| (policy.id, policy))
            .collect();
        let group_map: HashMap<PolicyGroupId, PolicyGroupProfile> = bundle
            .policy_groups
            .iter()
            .cloned()
            .map(|group| (group.id, group))
            .collect();
        let set_map: HashMap<PolicySetId, PolicySetProfile> = bundle
            .policy_sets
            .iter()
            .cloned()
            .map(|set| (set.id, set))
            ...
            for group_id in group_ids {
                ...
                for policy_id in policy_ids {
                    ...
                    candidates.push(PolicyCandidate {
                        ...
                        policy_set_name: set_map.get(set_id).map(|set| set.info.name.clone()),
                        policy_group_id: Some(group_id),
                        policy_group_name: group_map
                            .get(&group_id)
                            .map(|group| group.info.name.clone()),
                        policy: policy.clone(),
                    });
                }
            }
    ...
    fn resolve_policy_set_group_ids(
        step_type: &str,
        set_id: PolicySetId,
        set_map: &HashMap<PolicySetId, PolicySetProfile>,
    ...
    fn resolve_policy_group_policy_ids(
        step_type: &str,
        group_id: PolicyGroupId,
        group_map: &HashMap<PolicyGroupId, PolicyGroupProfile>,
    ...
    fn build_set_group_map(
        relations: &[PolicySetGroupLink],
    ) -> HashMap<PolicySetId, Vec<PolicyGroupId>> {
        let mut relation_map: HashMap<PolicySetId, Vec<PolicySetGroupLink>> = HashMap::new();
    ...
    fn build_group_policy_map(
        relations: &[PolicyGroupPolicyLink],
    ) -> HashMap<PolicyGroupId, Vec<PolicyId>> {
        let mut relation_map: HashMap<PolicyGroupId, Vec<PolicyGroupPolicyLink>> = HashMap::new();
        for relation in relations {
            relation_map
    ...
    fn resolves_policy_set_bindings_with_top_and_reverse() {
        ...
        set_map.insert(
            ...
            PolicySetProfile {
                ...
                order_index: 0,
                info: domain_script::PolicySetInfo {
                    name: "A".to_string(),
                    note: String::new(),
                },
            },
        ...
        for (id, order_index, name) in [(group_a, 0, "A"), (group_b, 1, "B")] {
            group_map.insert(
                ...
                PolicyGroupProfile {
                    ...
                    script_id: ScriptId::new_v7(),
                    order_index,
                    info: domain_script::PolicyGroupInfo {
                        name: name.to_string(),
                        note: String::new(),
                    },
                },
