impl ScriptExecutor {
    ...
    async fn resolve_policy_base_click_pos(&self, policy_id: PolicyId) -> ExecuteResult<u16> {
        ...
        bundle
            ...
            .map(|policy| policy.info.cur_pos)
            .ok_or_else(|| {
                Self::execute_error(
    ...
    async fn save_runtime_template_values(
        ...
        infra_sqlite::upsert_template_value(device_id, script_id, time_template_id, account_id, values_json.clone(), now)
            .await
            .map_err(|error| Self::execute_error("action.dropSetNext", error))?;
