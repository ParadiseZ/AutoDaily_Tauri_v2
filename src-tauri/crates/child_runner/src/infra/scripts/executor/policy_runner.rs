...
impl ScriptExecutor {
    ...
    async fn execute_policy_candidates(
        ...
        for (index, candidate) in candidates.into_iter().enumerate() {
            ...
            let policy_name = candidate.policy.info.name.clone();
            Log::debug(&format!(
            ...
            let before_action = candidate.policy.info.before_action.clone();
            let after_action = candidate.policy.info.after_action.clone();
            self.begin_active_policy_round_trace(
                candidate.policy.id,
                policy_name.clone(),
                candidate.policy.info.cur_pos,
            )
            .await;
