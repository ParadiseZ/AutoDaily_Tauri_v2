impl ScriptExecutor {
    ...
    fn collect_ocr_crop_entries(
        ...
        det_results
            ...
            .filter_map(|(idx, det_result)| {
                infra_vision::get_crop_image_rgba(
                    image, det_result,
                )
                .ok()
                .map(|crop| (idx, crop))
    ...
    pub(crate) fn build_capture_asset_signature(
        script_info: &domain_script::ScriptInfo,
    ) -> String {
        let mut hasher = XxHash3_64::default();
    ...
    pub(crate) fn build_text_rec_model_signature(
        ...
        match model {
            domain_vision::RecognizerType::PaddleCrnn(cfg) => {
    ...
    fn detector_asset_signature(
        model: Option<&domain_vision::DetectorType>,
    ) -> String {
        ...
        match model {
            domain_vision::DetectorType::Yolo11(cfg)
            | domain_vision::DetectorType::Yolo26(cfg) => format!(
                "model={};label={}",
                Self::base_model_asset_signature(&cfg.base_model),
                Self::optional_file_asset_signature(cfg.label_path.as_deref())
            ),
            domain_vision::DetectorType::PaddleDbNet(cfg) => {
                Self::base_model_asset_signature(&cfg.base_model)
            }
        }
    }
    ...
    fn recognizer_asset_signature(
        model: Option<&domain_vision::RecognizerType>,
    ) -> String {
        let Some(model) = model else {
            return "none".to_string();
        };
        ...
        match model {
            domain_vision::RecognizerType::PaddleCrnn(cfg) => format!(
                "model={};dict={}",
                Self::base_model_asset_signature(&cfg.base_model),
                Self::runtime_recognizer_dict_signature(cfg)
            ),
        }
    }

    fn runtime_recognizer_dict_signature(
        model: &domain_vision::PaddleRecCrnn,
    ) -> String {
        Self::resolved_path_asset_signature(
            infra_vision::resolve_recognizer_dict_path(model),
        )
    }

    fn base_model_asset_signature(
        model: &domain_vision::BaseModel,
    ) -> String {
        Self::resolved_path_asset_signature(
            infra_vision::resolve_model_path(model),
        )
    }
    ...
    fn resolved_path_asset_signature(
        path: infra_vision::VisionResult<std::path::PathBuf>,
    ) -> String {
        match path {
    ...
    async fn resolve_capture_timeout_ms(&self) -> u64 {
        ...
        match &device_config.cap_method {
            domain_device::CapMethod::Window {
                interface,
                frame_timeout_secs,
                ..
            } if matches!(
                interface,
                domain_device::WindowCaptureInterface::Dxgi
            ) =>
            {
    ...
    fn is_dxgi_capture_timeout_error(error: &domain_script::ScriptError) -> bool {
        matches!(
            error,
            domain_script::ScriptError::ExecuteErr { step_type, e }
    ...
    fn is_window_capture_bind_error(
        error: &domain_script::ScriptError,
    ) -> bool {
        matches!(
            error,
            domain_script::ScriptError::ExecuteErr { step_type, e }
    ...
    fn build_page_fingerprint(snapshot: &VisionSnapshot) -> String {
        ...
        for item in &snapshot.layout_items {
            hasher.write(&[match item.source {
                domain_vision::VisionLayoutSource::Ocr => 1,
                domain_vision::VisionLayoutSource::Det => 2,
            }]);
