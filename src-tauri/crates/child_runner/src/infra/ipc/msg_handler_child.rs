use crate::infra::context::runtime_control::{
    RunningStatus, clear_stop_request, get_ipc_client, get_scheduler, request_stop_execution,
    set_running_status, trigger_cancel,
};
...
use domain_device::{DeviceConfig, DevicePlatform};
...
use infra_device_runtime::{
    ...
    resolve_runtime_connect_config, try_get_device_ctx,
};
use infra_vision::dynamic_image_to_base64;
...
/// 子进程消息处理器
/// 处理来自主进程的命令消息
pub(crate) async fn handle_main_message(msg: IpcMessage) {
    match msg.payload {
...
async fn handle_capture_control(
    request_id: ad_kernel::ids::MessageId,
