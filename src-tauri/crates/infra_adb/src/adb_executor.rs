...
use crate::adb_command::{
    ADBCmdConv, ADBCommand, ADBCommandResult, BACK, HOME, click_cmd, input_text_cmd,
    long_click_and_swipe, long_click_cmd, sleep_cmd, start_activity_cmd, stop_app_cmd, swipe_cmd,
    swipe_duration_cmd,
};
use crate::adb_config::ADBConnectConfig;
use crate::adb_error::{AdbError, AdbResult};
...
pub(crate) struct ADBExecutor {
    ...
    pub(crate) cmds_after_conversion: Arc<Mutex<VecDeque<ADBCmdConv>>>,
    duration: Duration,
...
impl ADBExecutor {
        ...
        match cmd_low {
            ...
            ADBCommand::AwaitResult(command, sender) => {
                let result = match self.execute_adb_command_with_retry(command.as_ref()).await {
                    Ok(ADBCommandResult::Success) => Ok(()),
    ...
    fn is_successful_result(result: &AdbResult<ADBCommandResult>) -> bool {
        matches!(result, Ok(ADBCommandResult::Success))
    }
