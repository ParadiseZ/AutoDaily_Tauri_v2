...
use crate::Log;
use domain_device::{DeviceConfig, DeviceTransportKind};
use infra_adb::{ADBConnectConfig, AdbServeByIdentifier, AdbServerConfig};
use runner_protocol::message::ConnectionStatusKind;
...
pub(crate) async fn launch_device_with_progress(
    config: &DeviceConfig,
...
pub(crate) async fn wait_for_device_connection_with_progress(
    config: &DeviceConfig,
    on_status: impl Fn(ConnectionStatusKind, String),
) -> Result<ADBConnectConfig, String> {
...
pub async fn ensure_device_connection_with_progress(
    ...
    Log::info(&format!(
        ...
        config.transport_kind, config.connect_address, config.connect_identifier, config.exe_path
    ));
    on_status(
        ConnectionStatusKind::ShellProbeChecking,
        "[ launcher ] 尝试连接设备...）".to_string(),
    );
    let initial_error =
        match probe_device_config_connection_with_timeout(config, SINGLE_PROBE_TIMEOUT).await {
            Ok(runtime_connect) => {
                Log::info(&format!(
    ...
    on_status(
        ConnectionStatusKind::ShellProbeChecking,
        format!("[ launcher ] 连接设备失败：{}", initial_error),
    );
...
pub fn resolve_runtime_connect_config(config: &DeviceConfig) -> Result<ADBConnectConfig, String> {
    build_connection_candidates(config)
        .into_iter()
        .next()
        .ok_or_else(|| "[ launcher ] 未配置设备连接信息".to_string())
}
...
pub async fn probe_device_config_connection_with_timeout(
    ...
    let mut last_error = "[ launcher ] 未配置设备连接信息".to_string();
...
async fn probe_device_connection_with_timeout(
    ...
    match tokio::time::timeout(
        timeout,
        tokio::task::spawn_blocking(move || probe_device_connection(&runtime_connect)),
    )
    .await
    {
        Ok(Ok(result)) => result,
...
async fn wait_for_connection_ready_with_progress(
    ...
    loop {
        ...
        for runtime_connect in candidates {
            match probe_device_connection_with_timeout(
                runtime_connect.clone(),
        ...
        on_status(
            ConnectionStatusKind::ShellProbeChecking,
            format!(
                "[ launcher ] 连接设备失败（{}，第 {} 次，已等待 {} 秒）：{}",
                probe_context, attempt, elapsed_secs, last_error
            ),
        );
        if started_at.elapsed() >= timeout {
            return Err(format!(
                "[ launcher ] 设备连接失败：等待 {} 秒后仍未就绪，最后一次错误: {}",
                timeout.as_secs(),
                last_error
            ));
        }
...
fn resolve_wireless_mdns_direct_tcp(
    ...
    let identifier = config
        ...
        .ok_or_else(|| "[ launcher ] 未设置无线调试设备标识".to_string())?;
    let adb_path = config
        .adb_path
        .as_deref()
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "[ launcher ] 未设置 adb 程序路径".to_string())?;
    let server_connect = config
        .adb_server_connect
        .ok_or_else(|| "[ launcher ] 未设置 ADB server 地址".to_string())?;
...
pub(crate) fn probe_device_connection(runtime_connect: &ADBConnectConfig) -> Result<(), String> {
...
mod tests {
    ...
    fn emulator_identifier_mode_reuses_serve_by_identifier_candidate() {
        let mut config = DeviceConfig::default();
        config.emulator_connect_mode = domain_device::EmulatorConnectMode::Identifier;
    ...
    async fn emulator_without_exe_fails_after_initial_shell_probe() {
        ...
        assert!(error.contains("[ launcher ] 未配置模拟器启动程序"));
