use crate::Log;
use crate::device_runtime::{AndroidDeviceRuntime, DesktopDeviceRuntime, DeviceRuntime};
use ad_kernel::Point;
use domain_device::{
    CapMethod, DeviceConfig, DeviceOperation, DevicePlatform,
...
pub fn init_device_ctx(ctx: Arc<DeviceCtx>) -> Result<(), String> {
    DEVICE_CTX
        .set(ctx)
        .map_err(|_| "DeviceCtx already initialized".to_string())?;
    Ok(())
}
...
pub struct DeviceCtx {
    ...
    pub(crate) runtime: Arc<RwLock<DeviceRuntime>>,
}
