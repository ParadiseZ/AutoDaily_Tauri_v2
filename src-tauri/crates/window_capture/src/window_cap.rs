...
use image::RgbaImage;
use runtime_common::logging::log_trait::Log;
use std::{
    sync::Arc,
...
impl WindowInfo {
    ...
    pub async fn refresh_window(&self) -> bool {
        ...
        F: Fn(&Window) -> Result<RgbaImage, String>,
    {
        if let Some(window) = self.ensure_window().await {
            match capture_once(&window) {
                ...
                Err(error) => {
                    Log::warn(&format!("{label}失败，尝试重新绑定窗口: {error}"));
                }
            }
        }
        ...
        match capture_once(&window) {
            ...
            Err(error) => {
                Log::error(&format!("{label}失败：{error}"));
                Err(error)
            }
        }
    }

    pub fn is_dxgi_supported() -> bool {
        monitor_capture::is_supported()
    }

    pub async fn capture_image_via_gdi(&self) -> Option<RgbaImage> {
        self.capture_image_via_gdi_result().await.ok()
    }

    pub async fn capture_image_via_gdi_result(&self) -> Result<RgbaImage, String> {
    ...
    pub async fn capture_image_via_dxgi_region(&self) -> Option<RgbaImage> {
        self.capture_image_via_dxgi_region_result().await.ok()
    }

    pub async fn capture_image_via_dxgi_region_result(&self) -> Result<RgbaImage, String> {
        ...
        match self.effective_interface() {
            ...
            WindowCaptureInterface::Gdi => {
                if self.config.interface == WindowCaptureInterface::Dxgi {
                    Log::warn("DXGI 不可用，已回退为 GDI 窗口截图");
                }
    ...
    pub async fn capture_image(&self) -> Option<RgbaImage> {
        self.capture_image_result().await.ok()
    }
