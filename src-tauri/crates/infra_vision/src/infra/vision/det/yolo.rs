...
impl From<YoloDetConfig> for YoloDet {
    fn from(config: YoloDetConfig) -> Self {
        Self {
            base_model: config.base_model.into(),
            class_count: config.class_count,
            class_labels: HashMap::default(),
            confidence_thresh: config.confidence_thresh,
            iou_thresh: config.iou_thresh,
            label_path: config.label_path,
            txt_idx: config.txt_idx,
            postprocess_kind: config.postprocess_kind,
...
impl YoloDet {
    ...
    pub(crate) async fn load_labels(&mut self) -> VisionResult<()> {
