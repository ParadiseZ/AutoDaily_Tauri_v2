...
pub fn register_by_config(
    ...
    if !shortcut_conf.capture.is_empty() {
        match register_single_shortcut(&manager, &shortcut_conf.capture, capture_shortcut_handler, "截图") {
            Ok(msg) => {
                registered_count += 1;
                messages.push(msg);
            }
    ...
    if !shortcut_conf.toggle_window.is_empty() {
        match register_single_shortcut(&manager, &shortcut_conf.toggle_window, toggle_window_handler, "显示/隐藏窗口") {
            Ok(msg) => {
                registered_count += 1;
                messages.push(msg);
            }
    ...
    if !shortcut_conf.toggle_all_scripts.is_empty() {
        match register_single_shortcut(&manager, &shortcut_conf.toggle_all_scripts, toggle_all_scripts_handler, "运行/停止所有脚本") {
            Ok(msg) => {
                registered_count += 1;
                messages.push(msg);
            }
    ...
    F: Fn(&AppHandle, &Shortcut, ShortcutEvent) + Send + Sync + 'static,
{
    let shortcut = Shortcut::from_str(shortcut_str)
        .map_err(|e| AppError::ShortCutSetFailed{detail: format!("解析{}快捷键 '{}' 失败", description, shortcut_str),e:  e.to_string()})?;

    manager.on_shortcut(shortcut, handler)
        .map_err(|e|AppError::ShortCutSetFailed{detail: format!("绑定{}快捷键 '{}' 失败", description, shortcut_str),e:  e.to_string()})?;
...
pub fn unregister_all(app_handle : &AppHandle) -> AppResult<()>{
    app_handle.global_shortcut().unregister_all()
        .map_err(|e| AppError::ShortCutSetFailed{detail: "卸载所有快捷键失败".to_string(),e:  e.to_string()})?;
    Ok(())
}
...
/// 运行/停止所有脚本快捷键处理函数
pub fn toggle_all_scripts_handler(_app_handle: &AppHandle, _shortcut: &Shortcut, event: ShortcutEvent) {
    // 只在按键按下时执行操作，避免按下和松开都触发
    if event.state != ShortcutState::Pressed {
        return;
    }
...
pub fn capture_shortcut_handler(_app_handle: &AppHandle, _shortcut: &Shortcut, event: ShortcutEvent) {
    // 只在按键按下时执行操作，避免按下和松开都触发
    if event.state != ShortcutState::Pressed {
        return;
    }
