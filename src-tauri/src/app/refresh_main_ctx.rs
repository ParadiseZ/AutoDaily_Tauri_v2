...
pub async fn refresh_devices(device_id : DeviceId, device_conf : DeviceConfig) -> AppResult<()>{
    ...
    // 更新设备信息
    handle.state::<Arc<Mutex<DeviceConfMap>>>().lock().await.insert(device_id, device_conf.clone());

    // 写入文件
    let mut  devices= handle.state::<ConfigManager>().get_conf_mut::<DeviceConfMap>(DEVICES_CONFIG_PATH).await?;
    devices.config.insert(device_id,device_conf);
    Ok(())
}

pub async fn refresh_device_by_script_id(script_id: ScriptId,device_id : DeviceId)-> AppResult<()>{
    let handle = get_app_handle();
    let mut script_info = handle.state::<Arc<Mutex<ScriptManager>>>().lock().await.load_script_info(script_id)?;
...
pub async fn refresh_all(handle: &AppHandle) {
    ...
    // 以主上下文内的设备配置为准
    let _ = ctx.log_receiver.lock().await.refresh_from_config(&ctx.device_config);
}
