...
impl ChildProcessInitData {
    ...
    pub fn init_data_from_main(&self) ->  InitResult<()> {
        // 设置线程亲和性
        set_process_affinity(&self.cpu_cores).map_err(
            |e| InitError::InitChildCoreAffinity {e:e.to_string()}
        )?;
        ...
        //初始化ipc客户端
        init_ipc_client(Arc::new(self.device_id), self.log_level).map_err(|_| ChildProcessError::FailedToInitialize {e:"初始化ipc客户端失败".into_string()})?;
