use crate::constant::sys_conf_path::{
    DEVICES_CONFIG_PATH, EMAIL_CONFIG_PATH, LOG_CONFIG_PATH, SCRIPTS_CONFIG_PATH,
...
pub async fn init_conf_sync(conf_mgr: &State<'_, ConfigManager>) -> InitResult<()> {
    //系统设置
    conf_mgr
        .init_category::<SystemConfig>(SYSTEM_SETTINGS_PATH, BaseDirectory::AppConfig)
        .await
        .map_err(|e| InitError::InitMainConfigErr {
            category: "系统-系统设置".into(),
            e: e.to_string(),
        })?;
    //日志设置
...
pub fn init_conf_async() {
    tokio::spawn(async move {
        ...
        // 设备设置
        if let Err(e) = conf_mgr
            .init_category::<DeviceConfMap>(DEVICES_CONFIG_PATH, BaseDirectory::AppConfig)
            .await
        {
        ...
        // 脚本设置
        if let Err(e) = conf_mgr
            .init_category::<ScriptsConfig>(SCRIPTS_CONFIG_PATH, BaseDirectory::AppConfig)
            .await
        {
        ...
        // 通知设置
        if let Err(e) = conf_mgr
            .init_category::<EmailConfig>(EMAIL_CONFIG_PATH, BaseDirectory::AppConfig)
            .await
        {
