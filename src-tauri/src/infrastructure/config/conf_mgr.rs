...
pub struct ConfigManager {
    // 临时实现，保持向后兼容
    pub(crate) caches: Arc<RwLock<HashMap<String, (Arc<dyn Any + Send + Sync>, std::path::PathBuf)>>>,
}
...
impl ConfigManager{
    ...
    pub async fn init_category<C: ConfigCategory + 'static>(&self, category: &str,
                                                        base_dir : BaseDirectory ) -> ConfigResult<()> {
        // 使用新 API 获取配置文件路径
        ...
        let config = if path.exists() {
            let json = fs::read_to_string(&path)
                .map_err(|e| LoadErr{path: path.to_string_lossy().to_string(),e:e.to_string()})?;
            serde_json::from_str(&json)
                .map_err(|e|DeserializeErr{ path:path.to_string_lossy().to_string(),e : e.to_string()})?
        } else {
            // 创建默认配置
            C::default()
        };
    ...
    pub(crate) async fn get_conf<C: ConfigCategory + 'static + Clone>(&self, category: &str) -> ConfigResult<C> {
        let caches = self.caches.read().await;
        let (data, _) = caches.get(category)
            .ok_or_else(|e| NotInitErr { conf_category : category.into(),e })?;

        // 尝试转换为具体类型
        let config = data.downcast_ref::<C>()
            .ok_or_else(|e| CastErr{ e })?;

        Ok(config.clone())
    }

    pub async fn get_conf_mut<C: ConfigCategory + 'static>(&self, category: &str) -> ConfigResult<ConfigWriteGuard<'_, C>> {
    ...
    async fn save_category<C: ConfigCategory + 'static>(
        ...
        let json = serde_json::to_string_pretty(config_ref)
            .map_err(|e|SerializeErr{ path: path.clone(), e : e.to_string()})?;
        //.context("Failed to serialize config")?;

        tokio::fs::write(path, json)
            .await
            .map_err(|e| WriteErr{ path: path.clone(),e: e.to_string()})?;
    ...
    pub async fn get<C: ConfigCategory + 'static>(category: &str) -> C{
        match get_app_handle().state::<ConfigManager>().get_conf::<C>(category).await{
            Ok(category) => category,
            Err(e) =>{
                Log::info(&format!("获取配置{:?}失败：{},将采用生成的默认配置",category,e.to_string()));
                C::default()
            }
        }
    }
    ...
    pub async fn get_mut<C: ConfigCategory + 'static>(category: &str) -> ConfigWriteGuard<'_, C>{
        match get_app_handle().state::<ConfigManager>().get_conf_mut::<C>(category).await{
            Ok(category) => category,
            Err(e) =>{
                Log::info(&format!("获取配置{:?}失败：{},将采用生成的默认配置",category,e.to_string()));
                C::default()
