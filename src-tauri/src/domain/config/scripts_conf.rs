...
impl ConfigCategory for ScriptsConfig{
    fn default() -> Self {
        Self{
            dir: get_app_handle().path().app_config_dir().unwrap().join("scripts")
        }
    }
}
...
impl ScriptsConfig {
    pub async fn get_dir() -> PathBuf {
        ConfigManager::get::<ScriptsConfig>(SCRIPTS_CONFIG_PATH).await.dir
    }

    pub async fn set_dir(dir : PathBuf) -> ConfigResult<()> {
        ConfigManager::get_mut::<ScriptsConfig>(SCRIPTS_CONFIG_PATH).await.dir = dir;
        Ok(())
    }
}