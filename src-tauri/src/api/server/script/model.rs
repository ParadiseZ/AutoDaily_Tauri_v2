...
use crate::api::server::dto::{BackendApiRes, ScriptModelFileDto};
use crate::api::server::{app_error_message, trans_api_res};
use crate::app::app_error::AppResult;
use crate::app::config::scripts_conf::ScriptsConfig;
use crate::app::config::store::get_or_init_config;
use crate::app::constants::{APP_STORE, SCRIPTS_CONFIG_KEY};
use crate::infra::http_client::HttpClient;
use domain_script::RuntimeType;
use domain_script::ScriptProfile;
use domain_vision::{BaseModel, DetectorType, ModelSource, RecognizerType};
...
pub(super) fn collect_model_uploads(
    ...
    if let Some(spec) = detector_upload(
        script.info.img_det_model.as_ref(),
        scripts_root,
        IMG_DET_MODEL,
    )? {
        uploads.push(spec);
    ...
    if let Some(spec) = detector_upload(
        script.info.txt_det_model.as_ref(),
        scripts_root,
        TXT_DET_MODEL,
    )? {
        uploads.push(spec);
    ...
    if let Some(spec) = recognizer_upload(
        script.info.txt_rec_model.as_ref(),
        scripts_root,
        TXT_REC_MODEL,
    )? {
        uploads.push(spec);
...
pub(super) fn rewrite_script_model_paths_for_published(
    script: &mut ScriptProfile,
    script_id: &str,
) {
    rewrite_detector_model_path(
        &mut script.info.img_det_model,
        script_id,
        IMG_DET_MODEL.file_name,
    );
    rewrite_detector_model_path(
        &mut script.info.txt_det_model,
        script_id,
        TXT_DET_MODEL.file_name,
    );
    rewrite_recognizer_model_path(
        &mut script.info.txt_rec_model,
        script_id,
