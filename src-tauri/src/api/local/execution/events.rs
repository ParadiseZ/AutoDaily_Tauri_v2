use ad_kernel::ids::{AssignmentId, DeviceId, ExecutionId, ScriptId, SessionId, StepId, TaskId};
...
impl From<RuntimeProgressPhase> for DeviceRuntimeProgressPhase {
    fn from(value: RuntimeProgressPhase) -> Self {
        match value {
            ...
            RuntimeProgressPhase::Stopping => Self::Stopping,
...
mod tests {
    ...
    #[test]
    fn maps_runner_phases_to_ui_phases() {
        assert_eq!(
            DeviceLifecycleStatus::from(RuntimeLifecyclePhase::Stopping),
            DeviceLifecycleStatus::Stopping
        );
        assert_eq!(
            DeviceRuntimeProgressPhase::from(ConnectionStatusKind::DeviceConnected),
            DeviceRuntimeProgressPhase::DeviceConnected
        );
    }
}
