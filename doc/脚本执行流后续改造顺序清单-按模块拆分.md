...
- 当前仍未完成：
  - 当前仍保留一个未接线的 `debug_execute()` 旁路，容易继续制造语义漂移
  - `Email` 通知渠道仍未接入真实发送链
  - observe refresh 目前主要还是动作后 hook 位点，尚未沉淀成更稳定的观察证据链
  - `ColorCompare` 仍未定稿
    - 当前仍停在条件节点占位报错
    - 更合适的归属仍是数据处理/过滤能力
...
6. `ColorCompare` 不建议继续停在条件节点里，应后续转去数据处理/过滤模型
7. `Email` 通知继续保留为后续模块；`DesktopDeviceAdapter` 不属于本版本范围，不计入当前阶段收口
...
- 已完成：
  ...
  - 当前已接行为：
    ...
    - `StopExecution`
    - `RestartApp`
  - timeout 后会追加 progress event
  - `PauseExecution / StopExecution / RestartApp` 会先尝试写 checkpoint
- 仍未完成：
  - 当前 detector 主要覆盖“动作后链路”，还没有扩展到更完整的 progress signal
  - `Email` 通知渠道仍未接真实发送链
