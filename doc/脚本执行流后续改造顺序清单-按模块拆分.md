...
- `M6 Checkpoint 恢复执行` 不再继续推进，视为关闭并移出后续排期
- `RunRecoveryTask + recovery_task_id` 继续保留
  - 仅作为 timeout 动作落点
  - 不再代表统一恢复协议
- 临时运行语义更新为：
  - 若设备当前正在执行，则先确认并暂停
...
- 当前仍未完成：
  ...
  - 如果后续要扩展，再单独设计更复杂的“补跑 / 错过执行日补偿”规则
  - 调试运行目标 `FullScript / Task` 当前会强制注入 `everyRun` 的 task-cycle 覆盖
    - 调试运行不吃 `DeviceQueue` 这套正式调度跳过逻辑
    - 调试运行也不会写调度记录，只保留运行日志与 runtime event
  - 运行期 skip 语义不属于装配层：
    - 任务在执行中被 `SetState(... Skip=true)` 标记时，由 `task_states.skip_flag` 处理
    - timeout 选择 `SkipCurrentTask` 时，也是在执行期标记当前任务跳过

### 目标

- 继续深化 `ExecutionPlanAssembler`，把剩余的运行前决策继续往装配层收口。
...
- 当前已完成：
  ...
  - `resolve_task_cycle(...)`
- 后续建议继续补：
  - 如有必要，再单独扩展更复杂的补跑 / 错过执行日补偿规则
  - 调试目标与正式运行继续统一“装配入口 / 模板值结构 / scope 读取方式”
    - 但不统一调度记录比对语义
    - `DeviceQueue` 才做正式调度跳过
...
- 装配期跳过与执行期跳过边界清晰：
  - 装配期跳过只看 `task cycle + 最近一次 Success 调度记录`
  - 执行期跳过由运行时 `skip_flag` 承担
- `scheduler` 不再重复推导 plan mode 或 `record_schedule`。
...
- 后续增强项：
  - 当前 detector 已从“仅动作后观测点”扩展到部分非动作节点，但还没有覆盖完整执行循环
  - 前端对 timeout 结果的展示还比较粗
  - 仍需要继续验证“避免重复通知”和“长链路卡死”场景
  - 当前还没有充分覆盖的场景包括：
    - 卡在长时间条件求值、数据处理等纯计算节点里，但期间没有新的 action 或已接入的流程节点
    - 卡在动作前准备阶段，例如恢复装填、观测准备、首次分析前
    - `enter_step / enter_task`、checkpoint safe point 到达等非动作型推进信号还没有统一纳入 detector
...
- 已完成：
  ...
  - 前端已补恢复态投影
    - 可展示恢复阶段、恢复目标、恢复模式、降级原因
    - 执行完成/失败后会刷新 checkpoint 投影，避免残留陈旧“可恢复执行”提示

### 目标

- M6 收口为“安全点恢复 + 前端恢复态投影”。
- 不推进精确恢复；命中嵌套 step 时降级为 `FromTaskStart` 属于设计边界，不作为 M6 未完成项。
...
- 前端能看到恢复阶段、恢复目标、恢复模式和降级原因，不再只显示“可恢复执行”。

---

## M7：PolicyGroup / PolicySet 执行计划（关闭）
...
- 语义边界已确认：
  - 它们保留为调试运行目标；
  - 不进入 `DeviceQueue` 正式 task planner。

### 目标

- 文档与代码一致化，不再把它们写成“待进入真正执行计划”。
...
- 如果后续确实要进入正式计划节点，需要先补单独设计，而不是复用当前调试分支。
...
- 临时运行目标在设备已运行时：
  - 先弹确认；
  - 再发 `Pause + PrepareCheckpoint(manual)`；
  - 等 `CheckpointReady / RestartReady` 后切换到临时目标。
- 临时运行目标统一规则：
  - 强制按 `everyRun` 装配；
  - 不写正式调度记录；
  - 仍保留运行日志与 runtime event。
- checkpoint 超过 1 天默认失效：
  - 不再装填到 session；
  - 前端也不再展示旧恢复点。
