...
- 当前仍未完成：
  ...
  - `AddPolicies` 的正式替代模型仍未定义，当前仅保留兼容空操作
  - `RestartApp` timeout 行为仍缺少全局 activity 元数据
  - `Email` 通知渠道仍未接入真实发送链
  - `DesktopDeviceAdapter` 的真实动作实现
...
2. `AddPolicies` 如果要恢复，不要在现有兼容空操作上继续叠逻辑，应先重新定义运行时模型
3. `RestartApp` 若要落地，需要补脚本级全局启动元数据，不应在当前 `pkg_name` 字段上做隐式推断
4. `Email` 通知和 `DesktopDeviceAdapter` 继续保留为后续模块
...
- `GetState(step)` 已按兼容空操作收口，并告警要求迁移到条件节点
