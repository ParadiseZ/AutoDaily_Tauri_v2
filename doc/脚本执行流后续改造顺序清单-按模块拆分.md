...
- `M4` 已进入第一阶段：
  ...
  - 已引入 `DeviceAdapter` 平台分发骨架，Android 已接当前动作链，Desktop 仍占位
...
- 当前仍未完成：
  - `Link / AddPolicies`
  - `GetState(step)` 的兼容收口或模型裁剪
  - `DesktopDeviceAdapter` 的真实动作实现
  - `PolicyExecutionResult.rounds[*].pageFingerprints / actionSignatures / actions` 的真实记录
  - observe refresh / timeout detector / checkpoint safe point 的正式 hook

### 当前建议顺序

1. 先处理 `Link / AddPolicies`
2. 再决定 `GetState(step)` 是彻底裁剪还是只保留条件节点入口
3. 补 `PolicyExecutionResult.rounds[*]` 的动作与页面证据记录
4. `DesktopDeviceAdapter` 继续保留文档占位，本版本不实现
5. 最后才接 observe refresh / timeout detector / checkpoint safe point
