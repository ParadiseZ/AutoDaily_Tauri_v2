...
- 明确 `cur_pos` 是策略级命中游标，运行时应有非持久化覆盖值。
- 执行策略点击时，按 `cur_pos` 选择第 N 个符合条件的 OCR/Det 命中项。
- 增加步骤：调整策略当前位置，支持 `+1` / `-1` / 可选设置固定值。
- 该步骤目标应能选择一个或多个策略，至少先支持单个策略。
...
- 增加“轮转任务 UI 变量”步骤。
- 目标选择：任务 + 该任务下某个绑定 UI 字段/变量。
- 仅允许 select / radio 这类离散选项。
- 运行时把当前值切到下一个选项，并写回任务模板变量/用户配置值。
- 需要明确写回的是调试作用域、当前任务实例、还是持久化模板值；按你的描述应偏向持久化用户 UI 设置值。
...
1. `dropsetnext` 写回范围：是持久化到任务模板变量值，还是只影响当前运行 assignment？按你描述“避免下次再进入错误场景”，我理解为需要持久化。
...
4. 做 `dropsetnext` 对应的任务 UI 变量轮转。
5. 做策略 `cur_pos` 的运行时应用与 `posadd/posminus`。
6. 最后处理变量点击与策略 `cur_pos` 的冲突设计。
