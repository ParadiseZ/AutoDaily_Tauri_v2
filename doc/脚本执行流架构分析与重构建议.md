...
- 当前“执行层”已经从纯骨架进入半闭环，但还没有真正收口。
  ...
  - 当前仍未完成的是：checkpoint 恢复执行、`PolicyGroup / PolicySet` 运行目标、完整 timeout / notify 闭环。
...
- timeout 现在已经进入执行器动作后 hook，并且会直接触发 `NotifyOnly / SkipCurrentTask / RunRecoveryTask / PauseExecution / StopExecution / RestartApp`。
  ...
  - 当前还没有发展成完整 detector 的部分是：更多 progress signal、非动作路径的观测链和更完整的前端结果投影。
- 编辑器调试运行现在已经进入正式主链。
...
- 完成 timeout/action 的后续闭环。
  - 包括更稳定的 progress signal、避免重复通知、动作后的状态投影。
