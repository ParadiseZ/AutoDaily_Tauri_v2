...
- 当前仍未完成的是：更精细的 checkpoint 恢复执行、完整 timeout / notify 闭环、更完整的前进证据采集，以及更完整的调试 scope 语义。
...
- 当前模板覆盖值、恢复事件和设备级执行策略已经接入主线。
  ...
  - 当前仍未进入闭环的是：更完整的前进证据采集、通知渠道和 checkpoint 恢复执行。
...
- 编辑器调试运行和任务页正式运行已经共用同一条 `LoadSession -> Start -> scheduler.execute_script -> ScriptExecutor` 主链。
  ...
  - 当前非 `DeviceQueue` 目标装配的是临时 `RuntimeQueueItem`，`time_template_id / account_id / account_data_json` 仍为空；`template_values_json` 只补了运行时注入的 `everyRun` 覆盖，还不是完整作用域。
...
- `ExecutionPlanAssembler` 已经不只是占位，但仍然更像“任务过滤器”，而不是完整的运行前决策层。
  ...
  - 当前还没有覆盖恢复任务注入、调试运行与正式运行的统一判定，以及更复杂的补跑规则。
...
- checkpoint 已经从“可保存、可重装、可展示”的最小恢复链，推进到“可恢复 task / step 安全点执行”的闭环。
  - `DeviceQueue / FullScript / Task` 在 checkpoint 命中时，会真实裁剪待执行任务队列并恢复当前 task。
  - `ResumeMode::FromTaskStart / FromStepStart / FromNextStep` 已接进主链，但 `step_id` 当前只对任务顶层步骤生效。
  - 命中定义指纹不兼容、运行目标不兼容或不可恢复 task 时，会明确降级并清理失效 checkpoint。
  - 当前还没做的是分支/循环内部的精确恢复；命中嵌套 step 时会降级为 task start。
- timeout 现在已经进入执行器动作后 hook，并且会直接触发 `NotifyOnly / SkipCurrentTask / RunRecoveryTask / PauseExecution / StopExecution / RestartApp`。
  ...
  - 当前还没有发展成完整 detector 的部分是：更完整的前进证据采集、非动作路径的观测链和更完整的前端结果投影。
  - 当前没有充分覆盖的场景包括：
    - 长时间卡在条件求值、数据处理、循环判断，但期间没有新的 action
    - 卡在 `WaitMs` 等非动作步骤
    - 卡在动作前准备、恢复装填或首次观测分析阶段
    - 策略调试里停留在匹配/分析阶段，而不是立即产生命中 action
    - 循环迭代推进、checkpoint safe point 到达等非动作型推进信号还没有统一纳入 detector
- 编辑器调试运行现在已经进入正式主链。
...
- 继续深化 checkpoint 恢复执行。
  - 当前已支持 task 级安全点和任务顶层 step 恢复；
  - 仍未补的是分支/循环内部的精确恢复，以及更细的恢复阶段结果投影。
- 继续完善编辑器调试运行的作用域装配。
  - 当前已强制注入 `everyRun` 的 task-cycle 覆盖，并禁用调度记录写入；
