...
4. 在执行器整理后，继续收口 timeout detector 与 checkpoint 恢复执行。
   - timeout/action 已经接入 child 动作后链路，并扩展到 `WaitMs / While / ForEach / HandlePolicySet / HandlePolicy`。
   - checkpoint 已经进入安全点恢复执行；当前剩余的是剩余节点 evidence、作用域语义和结果投影收口。
...
- timeout 现在已经进入执行器动作后 hook，并且会直接触发 `NotifyOnly / SkipCurrentTask / RunRecoveryTask / PauseExecution / StopExecution / RestartApp`。
  ...
  - 当前没有充分覆盖的场景包括：
    ...
    - `task / step` 进入、checkpoint safe point 到达等非动作型推进信号还没有统一纳入 detector
- 编辑器调试运行现在已经进入正式主链。
...
- checkpoint 已能驱动真实的 task/step 安全点恢复执行，当前剩余的是恢复作用域与可视化投影继续补齐。
