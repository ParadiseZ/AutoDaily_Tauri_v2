...
- checkpoint 已经从“可保存、可重装、可展示”的最小恢复链，推进到“可恢复 task / step 安全点执行”的闭环。
  ...
  - 命中嵌套 step 时降级为 task start 是当前安全点恢复的既定边界，不继续推进精确恢复。
...
- 任务管理页现在开始区分“正式运行”和“临时运行”。
  ...
  - 设备已在运行时，前端会先确认，再发 `Pause + PrepareCheckpoint(manual)`，等恢复事件进入安全点后再切换目标
  - 临时运行仍强制 `everyRun`，不写正式调度记录，但保留运行日志与 runtime event
- 前端运行态仍偏“粗粒度投影”。
  - checkpoint 恢复态已能展示恢复阶段、恢复目标、恢复模式和降级原因；
...
- checkpoint 失效规则已收敛为“超过 1 天默认失效”。
  - 主进程在装填前会清理超时 checkpoint；
  - 前端拉取恢复点时也会同步过滤与清理，避免继续展示过期恢复点。
