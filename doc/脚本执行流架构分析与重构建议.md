...
- 当前模板覆盖值、恢复事件和设备级执行策略已经接入主线。
  ...
  - 当前仍未进入闭环的是：更完整的前进证据采集、timeout 结果投影，以及恢复执行的剩余边界收口。
...
- 主进程还应主导 checkpoint 重启流程。
  ...
  - 仍未做完的是把这条统一重启编排扩展到更多入口，以及恢复作用域/投影的剩余收口。
...
- 继续深化 `ExecutionPlanAssembler`。
  ...
  - 剩余的是恢复任务注入、更复杂的跳过/补跑规则，以及调试运行与正式运行更完整的判定一致性。
