...
| 编辑器运行入口 | `cmd_run_script_target` + 前端入口已落地 | 继续扩展 | 当前 `Task / Policy / PolicyGroup / PolicySet` 已进入调试主链，`FullScript` 仍未在编辑器选择器显式暴露 |
...
- `FullScript` 运行目标当前仍未在编辑器顶部选择器显式暴露，但底层 `RunTarget` 已支持。

### 验收标准

- 编辑器可以选择已暴露的调试目标运行，并且目标真正下发到 child 主链。
