...
本轮已删除“checkpoint / 恢复执行”目标与实现，后续阅读本文件时请以这条更新为准：
...
- 保留：
  ...
  - `recovery_task_id`
- 保留原因：
  - 这两个只服务于 timeout 动作落点
  - 不再承担通用恢复协议语义
- `cmd_restart_device_runtime` 已收敛成普通重启：
  - 停 child
  - 拉起 child
  - 重新装填 `DeviceQueue` session
- 任务管理页临时运行已改为“确认并暂停后直接切目标”，不再等待 checkpoint 安全点
- 本文后面所有 checkpoint 相关设计段落，应视为历史方案记录，不再纳入后续实现目标
...
| 状态事件 | `device-status` / `device-progress` / `device-schedule` / `device-error` / `child-log` | 保留并扩展 | 当前结构化 runtime event 已落地，后续补执行器内部细节 |
...
| 目标对象定义 | `ExecuteTarget` 与外层运行语义分裂 | 已统一为 `RunTarget`，main/child 只保留一套概念 |
...
- `ExecutionPlanAssembler`
  - 当前已把 bundle + queue item + run target 收口成真正可执行的 `ExecutionPlan`
  - 当前不再额外引入 `ExecutionPlanMode`，由 `ExecutionPlan` 变体本身承载运行语义
...
- child 侧 timeout detector 已额外覆盖部分非动作节点：
  - `WaitMs`
  - `While / ForEach`
  - `HandlePolicySet / HandlePolicy`
  - 策略调试候选扫描
- 尚未完全接完的是：
  - 条件求值、数据处理等纯计算节点的 timeout evidence
  - `task / step` 进入、checkpoint safe point 到达等统一推进信号
  - timeout 行为深化与恢复编排
...
- 命中嵌套 step 时降级为 task start 属于安全点恢复边界，不继续推进精确恢复。
- 前端已能投影恢复阶段、恢复目标、恢复模式、降级原因和 checkpoint 清理后的最新状态。
- checkpoint 失效规则已接入：
  - `updated_at` 视为恢复点设置时间；
  - 超过 1 天的 checkpoint 会在主进程装填前和前端查询时清理。
- 仍未完成的是把这条统一重启编排扩展到更多入口。
...
- `ExecutionPlanAssembler` 已接入：
  - `DeviceQueue / FullScript / Task` 统一先装配 `ExecutionPlan`
  - `ExecutionPlan` 当前区分：
    - `DeviceQueue(TaskSelection)`
    - `FullScript(TaskSelection)`
    - `Task(TaskSelection)`
    - `PolicyDebug`
  - `ExecutionPlanSummary` 与 `PlannedTask / SkippedTask.record_schedule` 已在装配期给出
  - `scheduler` 直接消费 `ExecutionPlanSummary`，不再自己推导 plan mode 或 `record_schedule`
...
- 调试运行已经统一走 `LoadSession -> Start -> scheduler.execute_script`。
  ...
  - `DeviceQueue` 正式运行已经真实消费 `template_values_json.taskSettings + variables`
  - 当前调试目标的 `build_debug_template_values_json()` 仍只注入最小 `everyRun` 覆盖，不等于完整模板/账号 scope
  - 非 `DeviceQueue` 目标不写 `device_script_schedules`，只保留运行日志与 runtime event
  - 编辑器控制台现在会吃所选设备的 child log，因此策略调试日志会直接显示在调试控制台里
  - 任务管理页新增的临时运行脚本 / 任务，也复用这套 `cmd_run_script_target` 语义：
    - 运行前不改 assignment 队列；
    - 运行中切换时先 `Pause + PrepareCheckpoint(manual)`；
    - 待 `CheckpointReady / RestartReady` 后再切换到临时目标。
...
- `M4 + M5` 当前已完成，后续重点转到调试 scope 语义补齐、恢复执行剩余边界和 timeout 增强。
...
5. 该项原本“明确延后”的 child timeout detector / 行为执行，现已进入主链并完成第一轮收口。
   - 动作后链路与部分非动作节点都已接入
   - 后续保留的是剩余节点补齐与结果投影深化，不再是“是否接入主链”的问题
...
6. 当前优先级最高的后续工作。
   ...
   - timeout 方向后续继续补：
     - 条件/数据处理节点 evidence
     - checkpoint safe point / task-step 进入信号
     - 更完整的运行中结果投影
