...
| 编辑器运行入口 | `cmd_run_script_target` + 前端入口已落地 | 继续扩展 | 当前 `Task` 已进入正式链路，`PolicyGroup / PolicySet` 仍显式拒绝 |
...
| 编辑器运行目标选择 | `Task` 进入统一运行接口，`PolicyGroup / PolicySet` 当前前后端一致地显式拒绝 |
...
- `TaskControl::GetState` 当前只保留给条件节点 `ConditionNode::TaskStatus` 复用目标/状态描述，不应再作为独立步骤模板暴露。
- 执行器如果收到旧数据里的 `GetState(step)`，会显式报兼容错误，而不是再定义新的“步骤输出语义”。
...
- 仍未完成的是 child 实际按 checkpoint 恢复执行，以及把这条统一重启编排扩展到更多入口。
...
- `ExecutionPlanAssembler` 已接入，当前实际支持：
  ...
  - `FullScript`
  - `Task`
- `PolicyGroup / PolicySet` 当前不会再走到 child 内才失败，而是由主进程和编辑器前置校验显式拒绝，没有做静默降级。
...
- 当前编辑器真实可用的运行目标：
  - `Task`
- `PolicyGroup / PolicySet` 当前已在编辑器和主进程侧显式提示“执行计划尚未接入”，不会再下发到 child。
...
6. 当前优先级最高的后续工作。
   - 继续完成 child 执行循环外的剩余清理：
     ...
     - `PolicyGroup / PolicySet` 的真正非执行器组织落地，而不是当前的显式拒绝
   - 等执行器结构重新稳定后，再接：
     - timeout detector
     - timeout action
     - 运行中通知策略
