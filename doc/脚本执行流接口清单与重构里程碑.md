...
- 当前代码已先落一版保守实现：
  ...
  - 如果改了 `cores`，主进程会先发 `PrepareCheckpoint`、等待 checkpoint 刷新，再重启 child 并重新装填 session
...
| `device-recovery` | `RuntimeRecoveryEvent` | 当前已落地的恢复事件，承载 checkpoint preparing / ready / loaded |
...
- browser mock 已同步这套 `RunRecoveryTask` 校验，避免本地 mock 与真实后端行为漂移。

说明：

- 这一阶段当前已经覆盖到“合同 + 配置入口 + 主进程校验”。
- 尚未接入的是 child 执行循环里的 timeout detector 与行为执行。
...
- 在线设备保存配置时，`cores` 变更已经会触发“等待 checkpoint 刷新 -> 重启 child -> reload session”，`execution_policy` 变更会触发 session 热更新。
- 仍未完成的是显式 `RestartReady` 握手、统一的主进程重启编排命令，以及 child 实际按 checkpoint 恢复执行。
...
- child 收到 `PrepareCheckpoint` 后，能在安全点写 checkpoint 并投递 `device-recovery`。
...
- `ExecutionPlanAssembler` 已接入，当前实际支持：
  - `DeviceQueue`
  - `FullScript`
  - `Task`
- `PolicyGroup / PolicySet` 仍明确返回“尚未接入执行计划”，没有做静默降级。
...
- `ScheduleJournal` 已开始把 task 结果 append 到 `device_script_schedules`。
...
- 运行一个脚本时，能写入 `device_script_schedules`。
...
- 当前编辑器真实可用的运行目标：
  - `Task`
- `PolicyGroup / PolicySet` 按统一链路下发，但 child 执行计划当前仍会明确返回“尚未接入”。
- `policy` 单项运行仍未接入，前端会直接提示不支持。
...
6. 当前优先级最高的后续工作。
   - 继续完成 child 执行循环外的剩余清理：
     - `RestartReady` 显式握手
     - 统一的主进程“checkpoint 后重启”命令，替代当前保存设备时的内嵌编排
     - `PolicyGroup / PolicySet` 的非执行器外层组织收口
   - 等执行器结构重新稳定后，再接：
     - timeout detector
     - timeout action
     - 运行中通知策略
