...
- 设备动作和策略处理仍然只是显式边界：
  ...
  - `LaunchApp / StopApp / Reboot`
    ...
    - `LaunchApp` 因当前只有 `pkg_name`、缺少明确 activity / launch target，已回退为显式未接入
...
- 设备动作已经进入 `dispatch_action()`，其中 `Capture`、点位类 `Click / Swipe`、`StopApp / Reboot` 已开始执行真实设备动作；`LaunchApp` 以及文字/标签类动作和策略执行仍未接入。
...
- `Action`
  ...
  - `LaunchApp` 因缺少 activity / launch target 暂不执行隐式启动
