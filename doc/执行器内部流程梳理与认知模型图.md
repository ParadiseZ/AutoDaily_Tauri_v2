...
- 设备动作已经进入 `dispatch_action()`，其中 `Capture`、`Click / Swipe`、`LaunchApp / StopApp / Reboot` 已开始执行真实设备动作；这些动作当前先经 `DeviceAdapter` 分发到 Android 适配器。文字/标签类动作现在会消费 `input_var` 中的 OCR/DET 结果集并按 bbox 中心点执行；策略执行仍未接入。
...
### 1. 执行器对外已经在主链上，但对内仍是“基础动作已接通、策略执行未接通”的骨架

结果是：

- 调度器已经把它当正式执行器使用
- 纯逻辑节点已经开始真实执行
- 但策略执行和更完整的观察闭环仍未接入真实适配器

所以现在的真实状态不再是“全空壳”，而是“逻辑执行骨架已成型，基础设备动作已通过 `DeviceAdapter` 部分接通，结果变量驱动的文字/标签动作也已接通，但策略执行和观察闭环仍缺适配器”。
